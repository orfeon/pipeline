...
""";
     ...
     * seller's Jan 1 row is not visible yet (null), on Feb 1 all three earlier rows are, and the composed value is the
    ...
    public void testOffsetOverOutcomeBaseline() throws java.io.IOException {
        ...
        final Map<String, MCollection> outputs = MPipeline.apply(pipeline, Config.load(SOURCE_CONFIG + config));
        final Schema schema = outputs.get("features").getSchema();
        Assertions.assertEquals("windowShift", schema.getField("f_enc__seller_id__count").getOptions().get("feature.status"));
        Assertions.assertEquals("PT144H38M", schema.getField("f_enc__seller_id__count").getOptions().get("feature.windowShift"));
        ...
        PAssert.that(outputs.get("features").getCollection()).satisfies(rows -> {
            final Map<String, MElement> byKey = new HashMap<>();
            for (final MElement row : rows) byKey.put(row.getAsString("session_id") + "/" + row.getAsString("seller_id"), row);
            // Jan 3: the Jan 1 settlement is not known yet — no history, not even a count
            Assertions.assertNull(byKey.get("B/s1").getPrimitiveValue("f_enc__seller_id__e2__mean"));
            Assertions.assertEquals(0L, byKey.get("B/s1").getAsLong("f_enc__seller_id__count"));
            // Jan 20: Jan 1 and Jan 3 have settled
