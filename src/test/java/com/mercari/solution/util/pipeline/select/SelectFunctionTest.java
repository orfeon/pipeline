...
public class SelectFunctionTest {
    ...
    public void testUuidCast() {
        ...
        Assertions.assertEquals(Schema.Type.uuid, outputSchema.getField("uuidId").getFieldType().getType());
        Assertions.assertEquals(Schema.Type.string, outputSchema.getField("textId").getFieldType().getType());
        ...
        Assertions.assertEquals(uuid, results.get("uuidId"));
        Assertions.assertEquals(uuid, results.get("textId"));

        final org.apache.avro.Schema avroSchema = ElementToAvroConverter.convertSchema(outputSchema.getFields());
        Assertions.assertEquals(LogicalTypes.uuid(), AvroSchemaUtil.unnestUnion(avroSchema.getField("uuidId").schema()).getLogicalType());
        Assertions.assertNull(AvroSchemaUtil.unnestUnion(avroSchema.getField("textId").schema()).getLogicalType());
        Assertions.assertEquals(Schema.Type.uuid,
    ...
    public void testUuidAndBytesCast() {
        ...
        Assertions.assertEquals(uuid, results.get("uuidId"));
        Assertions.assertEquals(16, ((ByteBuffer) results.get("bytesId")).remaining());
        Assertions.assertArrayEquals(bytes.array(), ((ByteBuffer) results.get("bytesId")).array());
    }
