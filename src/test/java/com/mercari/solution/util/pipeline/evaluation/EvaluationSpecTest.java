...
public class EvaluationSpecTest {
    ...
    public void testPredictionAndFamilyRules() {
        ...
        Assertions.assertTrue(error("{group: g, label: y, baseline: b, time: t, predictions: [{name: A, score: s, offset: {field: qa, form: inverseShare, invalid: dropRow}}], " + EvaluationScorerTest.SPLITS + "}").contains("offset.invalid is unknown"));
        parse("{group: g, label: y, baseline: b, time: t, predictions: [{name: A, score: s, offset: {field: qa}, offsetScale: null}], " + EvaluationScorerTest.SPLITS + "}").resolve(EvaluationScorerTest.SCHEMA, null);
