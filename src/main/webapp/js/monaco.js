...
function buildPipelineSchema() {
    ...
    actionModule.properties.failWhen = { type: ['string', 'object', 'array', 'boolean'], description: 'Post-execution condition: fail the firing when it matches (SQL-like text over payload.*, state, jobId)' };
    actionModule.properties.skipWhen = { type: ['string', 'object', 'array', 'boolean'], description: 'Post-execution condition: emit state SKIPPED when it matches' };
    return {
        type: 'object',
        properties: {
