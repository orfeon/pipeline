...
|------------------------------|----------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| project                      | String         | The project ID for your Google Cloud project. The project is required if you want to run your pipeline using the Dataflow managed service. When not set, it is resolved from the execution environment in this order: the gcloud config (Beam's own inference), the `GOOGLE_CLOUD_PROJECT` / `GCLOUD_PROJECT` environment variable or system property, the project of the credentials (a service-account key's `project_id`, or the quota project), and — only when running on Google Cloud (`MERCARI_PIPELINE_ON_GCP` overrides the detection) — the google-cloud client resolution, which ends at the GCE metadata server (Cloud Run / GCE). BigQuery query and load jobs, and the `datastore` sink, run in this project unless a more specific option (e.g. `bigquery.bigqueryProject`) is set. Off Google Cloud (e.g. EMR) with no project in the credentials, set this or `GOOGLE_CLOUD_PROJECT` if any GCP module is used.                                                                                                       |
