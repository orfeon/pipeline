...
| `get-job-logs` | `getJobLogs` | Cloud Logging entries of the job (`minSeverity`, `contains`, `limit`; latest first) — the INFO / WARNING context around an error, the feature plan report a job logs at startup, progress messages. Cloud Run container stdout carries severity DEFAULT, so for `runner: direct` the DEBUG / INFO thresholds return all lines (stderr maps to ERROR) |
