...
time-decayed features (`DECAY_SUM` / `DECAY_AVG` with `asOf = i.__timestamp`),
and **per-key model refits** — `LINEAR_REG` / `BAYES_LINREG` +
`LINREG_PREDICT` / `BAYES_PREDICT` re-fit on the visible history per element
(order-robust: a late element corrects later evaluations automatically),
`SEQ_HOLT` forecasts, `SEQ_PAGE_HINKLEY` / `SEQ_CUSUM` retrain triggers.
...
| `SEQ_PAGE_HINKLEY(rows, field, delta, lambda)` | **Drift detection** (two-sided Page–Hinkley) over an ordered sequence: returns the **1-based index** of the first position where the cumulative deviation from the running mean (minus tolerance `delta` ≥ 0) leaves its running extremum by more than `lambda` > 0, or `0` when no change was detected (`INTEGER`). Re-evaluated over the visible history it is order-robust — the per-key "should this model retrain?" trigger (route matches to a Pub/Sub sink via a session output). `field` follows the `SEQ_FOLD` field spec; NULL values are skipped. |
| `SEQ_CUSUM(rows, field, target, slack, threshold)` | **Drift detection** (two-sided tabular CUSUM) around a known `target` level: `S⁺ = max(0, S⁺ + x - target - slack)` and mirrored `S⁻`; returns the 1-based index where either side first exceeds `threshold` > 0, else `0` (`INTEGER`). `slack` (≥ 0, the usual `k`) absorbs acceptable wander. Same field spec / NULL handling as `SEQ_PAGE_HINKLEY`. |
| `SEQ_EXP_SMOOTH(rows, field, alpha)` | **Simple exponential smoothing** over an ordered sequence: returns the final smoothed level = the one-step-ahead forecast (`DOUBLE`); `alpha` ∈ (0, 1]. NULL values are skipped; an empty/all-NULL sequence yields NULL. Same field spec as `SEQ_FOLD`. |
| `SEQ_HOLT(rows, field, alpha, beta [, horizon])` | **Holt linear-trend forecast** `horizon` ≥ 1 steps ahead (default 1, `DOUBLE`): level weight `alpha` ∈ (0, 1], trend weight `beta` ∈ (0, 1]. The lightweight per-key demand-forecast model — `SEQ_HOLT(SEQ_COLLECT(h.__timestamp, h.demand), 'v1', 0.4, 0.2, 7)` forecasts 7 steps out from a buffer/lookup history. A single-value sequence has no trend and forecasts that value. |
...
| `DECAY_SUM(value, ts, halfLife, asOf)` / `DECAY_AVG(value, ts, halfLife, asOf)` | **Aggregates**: exponentially **time-decayed** sum / weighted mean (`DOUBLE`) — each row weighs `2^(-(asOf - ts) / halfLife)` (half per `halfLife` millis of age; day-time `INTERVAL` literals work as `halfLife`). The reference time is explicit (`asOf`, usually `i.__timestamp` — include it in `GROUP BY`), so the functions behave identically over buffer and external-source history. Rows with a NULL `value`/`ts` are skipped; all-NULL groups yield NULL. Mergeable, constant memory. |
| `DECAY_COUNT(ts, halfLife, asOf)` | **Aggregate**: the decayed event count `Σ 2^(-(asOf - ts)/halfLife)` (`DOUBLE`) — an event-rate feature. NULL over an all-NULL group (Calcite's strict user-aggregate convention) — wrap in `COALESCE(…, 0.0)` for COUNT semantics. |
| `BETA_SAMPLE(alpha, beta, seed)` | A **deterministic Beta-distribution sample** in `(0, 1)` (`DOUBLE`) — the Thompson-sampling primitive: keep per-arm success/failure counts in a buffer or lookup table and sample the posterior at decision time. The draw is a pure function of the arguments (repeatable under bundle retries): derive `seed` from element data (`UNIX_MILLIS(i.__timestamp) + v.variantNo`) and vary it per row for independent draws. NULL → NULL; shapes must be > 0. |
...
| `LINEAR_REG(y, xs [, lambda])` | **Aggregate**: multivariate linear regression of `y` on the feature vector `xs` (`ARRAY[x1, x2, ...]`, same length every row); an intercept term is prepended automatically, so the coefficients are `[b0, b1, ..., bk]`. The optional `lambda` ≥ 0 is a **ridge** penalty on the non-intercept coefficients — per-key regularized refits over small histories. Accumulates the Gram matrix (constant memory per group) and solves at result time. Rows with NULL `y`/`xs` are skipped. The raw result is an opaque collection — feed it to `LINREG_PREDICT` or wrap in `AS_DOUBLE_ARRAY` to project it. |
| `LINREG_PREDICT(model, features)` | Apply a `LINEAR_REG` result (or any `[intercept, b1, ...]` coefficient array) to a feature vector (`DOUBLE`) — **train on the history rows and predict the current element in one statement**: `LINREG_PREDICT(LINEAR_REG(h.y, ARRAY[h.x1, h.x2]), ARRAY[i.x1, i.x2])` under `GROUP BY i.x1, i.x2` (per-element evaluation: grouping by the current element's columns keeps them in scope alongside the aggregate). NULL model (empty fit) → NULL. |
| `BAYES_LINREG(y, xs, priorPrecision [, noiseVar])` | **Aggregate**: Bayesian linear regression with an isotropic Gaussian prior `N(0, priorPrecision⁻¹ I)` over all coefficients (intercept included). With `noiseVar` the posterior is exact; without it the noise variance is estimated from the ridge-fit residuals (`RSS / max(1, n − k)`). The opaque result carries the posterior mean **and covariance** — consume with `BAYES_PREDICT`. Same accumulation/NULL rules as `LINEAR_REG`. |
| `BAYES_PREDICT(model, features)` | Posterior-predictive `[mean, sd]` (`ARRAY<DOUBLE>`, observation noise included) of a `BAYES_LINREG` model at a feature vector — **uncertainty-aware forecasts** for downstream decisions (order quantities, alert thresholds). NULL model → NULL. |
