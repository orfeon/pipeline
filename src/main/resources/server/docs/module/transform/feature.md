...
fit is unchanged: its statistics are an artifact of the fit boundary). Such a baseline is a valid offset
even though the current row cannot see its own value yet — so `baselines[].emit` of it, and an
`output.roles.baseline` naming it, stay `availability.violation`. The offset is an additive term on the
shrinkage scale:
