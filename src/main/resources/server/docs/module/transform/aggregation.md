...
| filter         | optional | [Filter](../common/filter.md)                           | Filter condition applied to the aggregation results, **before** this module's own `select` / `flattenField` (conditions must reference aggregation output fields, not fields computed by `select` — unlike the [select](select.md) transform's `outputFilter`, which applies after its `select`). Records that do not match are discarded.                          |
