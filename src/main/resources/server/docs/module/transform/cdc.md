...
- applied to a destination by an apply-capable sink (`bigquery`, `spanner` or `postgres` sink with
  `cdc: true`; the spanner sink can apply each source transaction atomically, the postgres sink
  guards against stale replays with a sequence column),
