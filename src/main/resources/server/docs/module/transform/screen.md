...
- Degenerate columns (fewer than two observed rows, a constant within every group, or an information `H` below
  a relative floor of the column's raw second moment — 1e-20 for `groupedMultinomial`, whose centring is exact
  per unit, 1e-12 for the row families, whose centring is a difference of moment sums) get `est_gain = 0` and
  `degenerate = true`.
...
the fill under which the missing row adds nothing to the unit's centred design, the rule the candidate columns already
follow) by Newton's method with an L2 penalty on the
...
`block:<name>`, `evidence:<declared|measured>`, `kind:<kind>` (the origin tag of a pass-through input field itself —
a derived column has no kind, only `derivedFrom`). Lineage is read from the input schema when the feature transform is
