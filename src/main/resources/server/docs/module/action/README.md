...
Available services: [bigquery](bigquery.md) · [vertexai](vertexai.md) · [storage](storage.md) · [tasks](tasks.md) · [http](http.md) · [dataflow](dataflow.md) · [build](build.md)
...
| build | `builds.create`, `builds.get`, `builds.list`, `builds.wait`, `builds.cancel`, `triggers.run` |
| vertexai | `batchPredictionJobs.create`, `batchPredictionJobs.get`, `batchPredictionJobs.list`, `batchPredictionJobs.wait`, `batchPredictionJobs.cancel`, `models.generateContent` |
...
|------------|-----------|--------------------------------------------------------------------|
| service    | STRING    | The action service (`bigquery`, `vertexai`, `storage`, `tasks`, `http`, `dataflow`, `build`). |
...
- Execution is **at-least-once**: Beam may retry a bundle and re-invoke the action, and `retry` re-invokes it as well. The bigquery service submits jobs idempotently (deterministic job ids, resubmitting under a `-r<n>` suffix only when the earlier attempt failed transiently); the vertexai service dedupes `batchPredictionJobs.create` on a deterministic `displayName` (a queued / running / succeeded job with that name is adopted) — without one a retry submits a second job; the dataflow service is idempotent only with a deterministic `jobName` (an active job with that name is adopted) — without one, a retried launch starts a second job; the build service dedupes `builds.create` on deterministic `tags` (a queued / working / succeeded build with the same tags is adopted) — without tags a retry starts a second build.
...
| module       | required | String          | The action service: `bigquery`, `vertexai`, `storage`, `tasks`, `http`, `dataflow`, `build`. |
| operation    | conditionally required | String | Which operation of the service to execute — see [Operation](#operation). Required for services that declare operations (bigquery, tasks, vertexai, dataflow, build); must be omitted for single-operation services (storage, http). |
