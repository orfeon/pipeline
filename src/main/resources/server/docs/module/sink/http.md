...
| `oauth2`   | `grant` (`clientCredentials` default \| `jwtBearer` \| `refreshToken`), `tokenUrl`, `scope`, `audience`, `refreshBeforeSeconds` (default 60); clientCredentials: `clientId`, `clientSecret`; jwtBearer: `issuer`, `subject` (default issuer), `privateKey` (PKCS#8 PEM, RS256), `keyId`, `jwtLifetimeMinutes` (default 60); refreshToken: `refreshToken`, `clientId`, `clientSecret` (optional — omitted for public clients) | OAuth2 token endpoint exchange. `clientCredentials` posts client id/secret (HTTP basic); `jwtBearer` (RFC 7523 — Salesforce JWT flow, Box, Adobe, custom IdPs) signs a JWT (`iss`/`sub`/`aud` = `audience` or `tokenUrl`/`scope`/`iat`/`exp`) and posts it as the assertion; `refreshToken` (RFC 6749 §6 — user-delegated APIs such as Google Drive/Ads, HubSpot, Salesforce web flow) exchanges a long-lived refresh token obtained out of band, and keeps a rotated refresh token returned by the server for the worker's lifetime. The access token is cached until `expires_in` − `refreshBeforeSeconds`. |
