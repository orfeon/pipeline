...
| password | conditional required | String | User password to access the database. Required when `user` is specified. You can also specify a Secret Manager resource name like `projects/{myproj}/secrets/{mysecret}/versions/latest`. No need to specify if the service account is used as `user`. |
