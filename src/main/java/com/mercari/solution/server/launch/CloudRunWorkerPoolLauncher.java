...
/**
 * {@code direct/cloudRunWorkerPool} and {@code prism/cloudRunWorkerPool}: deploy the {@code direct} /
 * {@code prism} image as a Cloud Run Worker Pool running this config (streaming / long-running
 * pipelines). One launch creates one worker pool;
...
public class CloudRunWorkerPoolLauncher implements Launcher {
    ...
    /** @param runner {@code direct} or {@code prism}: the image profile the pool is deployed from. */
    public CloudRunWorkerPoolLauncher(final String runner, final CloudRunUtil cloudRun, final ConfigStager stager) {
        this.runner = runner;
    ...
    public JsonObject launch(final LaunchRequest request) throws Exception {
        ...
        final String image = defaults.resolve(runner, KEY_IMAGE, request.param("image"))
                .orElseThrow(() -> new IllegalArgumentException(runner + " image is required to deploy a worker pool: specify image"
