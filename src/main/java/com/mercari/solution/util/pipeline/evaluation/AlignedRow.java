...
/**
 * One row of a scored unit as the calibration tables read it: the split, the normalised label ỹ, the
 * baseline mean p, every prediction set's mean q (baseline form: a share or a probability), the calibration
 * fields' values and the utility (NaN = none).
 */
public final class AlignedRow implements Serializable {
    ...
    public AlignedRow(final String split, final double label, final double baseline, final double[] predictions, final double[] fields, final double utility) {
        this.split = split;
        this.label = label;
    ...
    private static class RowCoder extends AtomicCoder<AlignedRow> {
        ...
        public AlignedRow decode(final InputStream in) throws CoderException, IOException {
            ...
            return new AlignedRow(split, label, baseline, predictions, fields, utility);
        }
    }
}
