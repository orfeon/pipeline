...
public final class OperatorCatalog {
    ...
    static {
        ...
        register(Scope.sequence, "rating", InputKind.numeric, F64, false, "sequential rating of the entity from the contests it took part in (method " + String.join(" | ", Rating.METHODS) + " - gaussian conditions the whole contest at once on its outcomes as margins, its prior and beta in the outcome's units; field = the outcome, context = the contest; funcs mu | sigma | count | delta | deviation | z; with = the other members of a team, team = its readouts mu | sigma | count | deviation; bradleyTerry pairs " + String.join(" | ", Rating.PAIRS) + "; tauPer = the period tau is the drift of instead of one contest): every contest moves all its players at once, so the state is not mergeable - one replay per pool (global key, or the partition of a reduced window filter)");
