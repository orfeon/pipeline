...
public final class OperatorCatalog {
    ...
    static {
        ...
        register(Scope.sequence, "rating", InputKind.numeric, F64, false, "sequential rating of the entity from the contests it took part in (method elo | bradleyTerry | plackettLuce; field = the outcome, context = the contest; funcs mu | sigma | count | delta): every contest moves all its players at once, so the state is not mergeable — one replay per pool (global key, or the partition of a reduced window filter)");
