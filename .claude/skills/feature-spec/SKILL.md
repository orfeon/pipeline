...
| out-of-fold encodings for time series (no neighbouring labels in the fit) | `fit: {mode: fold, blocks: {...}, fold: {by: time, purge, embargo}}` | purge defaults to the target label's horizon; a cross-fit (later blocks are read) |
