...
| decorrelated low-rank summary of several numeric features (e.g. a lag window) | `population`, `type: svd` (`inputs`, `rank`) | static, or `fit: {mode: forward, blocks, window}` for walk-forward components under drift; fitted from sufficient statistics only |
