...
| a value **neutralised** against other fields inside the event (what the score says beyond the market), or "within the first k" probabilities from win probabilities | `context` ops `residualize` (`against: [...]`) / `harville` (`top`, `discount`) | `residualize` needs `p + 2` complete rows per group (null otherwise); `harville` is cubic for `top: 3` (`maxGroupSize`, default 64) |
...
| numeric coordinates for a high-cardinality categorical state, without a target | `population`, `type: spectralEmbedding` (`sequenceOf`, `cooccur.window`, `rank`) | values that precede / follow the same values land close; `of: previous` when the field is an outcome (the row's own value would be a violation), `of: [current, previous]` for both values from the one fit (`<name>_<k>` and `<name>_prev_<k>`); static or `fit.mode: forward`; vocabulary capped by `maxValues` (default 256), a value beyond it reads null |
...
Every fitted block (`encoding` outside `expanding`, `svd`, `quantileTransform`, `smooth`, `discretize`, `factorization`)
...
- **`rating` split into components** (a seller's overall level, its level in this category, its pairing with
  this agent): members are any `entities[].name`, and entities take composite keys, so
  `with: [{entity: sellerCategory, mu: 0, sigma: 2}, {entity: sellerAgent, mu: 0, sigma: 1.5}, ...]` with
  `sellerCategory: {keys: [seller_id, category]}` rates the row as the sum of those components (a random-effects
  ...
  that pairing has run. Only sums are identified, so read components in sums or relative to the contest.
