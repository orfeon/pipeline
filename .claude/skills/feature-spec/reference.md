...
| `output` | optional | `prefix`, `nullPolicy: keep \| fillZero \| indicator`, `exclude: [globs / selectors]`, `groupBy: <context>`, `parentFields: [...]`, `childName` (default `rows`), `passThrough: all \| keys \| none`, `roles: {group, time, entity, label, baseline, weight}`, `include: [names] \| <uri> \| {from: [uri, ...], mode: union \| intersection}` (projection; replaces `exclude`), `manifest: <uri>` |
...
(objects with `name` allowed) or one name per line; `{from: [uri, ...], mode}` combines several such files —
`union` (default: every name, first-appearance order) or `intersection` (the first file's names present in all). Role columns (a baseline's `emit` copy, a label
...
| `vector` | `input` (an `array<float64>` field), `funcs: [length, sum, mean, std, min, max, argmin, argmax, first, last, slope, norm, polyfit, vector]`; optional steps applied in this order: `slice: {from, to}` (negative = from the end, clamped), `diff: <order>`, `normalize: sum \| mean \| l2 \| zscore`, `resample: <length>` (linear interpolation onto a fixed length) / `pad: {length, mode: edge \| zero, side: end \| start}` (never truncates); `position: index \| unit` (slope / polyfit), `degree: 1..5` (polyfit, default 2), `coefficients: [1, 2]` (the polyfit coefficients to emit) | `<name>_<func>` (float64; `length` / `argmin` / `argmax` int64), `polyfit` → `<name>_poly0..poly<degree>` (or the listed ones), `vector` → `<name>_vector` array<float64> (the stepped vector: an array `svd` input at a fixed length). Inherits the array field's availability. Null array or a null / NaN element → every readout null; an undefined readout (too few elements, zero denominator) → null; an empty vector has `length` 0 only. A `repeated` field without a value arrives as the empty array |
...
| `softmax` | numeric `field` (score); `offset: <baselines[].name or column>` in probability space (`offsetScale: log` takes exp first), `temperature` (> 0, default 1) or `temperatureFrom: <uri>` (number or JSON `{temperature}` / `{T}`; outside the plan hash, in the manifest `externals`), `scoreNull: zero \| null` (an unquoted YAML `null` is the keyword too) | float64: `w·exp(f/T)` normalised over the group; null offset → null row out of the denominator, offset 0 → 0; inherits the offset's `validFor`; `nullPolicy: indicator` adds `_isnull` and `_scoreNull` |
| `residualize` | numeric `field`, `against: <field>` or `[a, b]` (numeric fields / columns — not `on`, a YAML 1.1 boolean) | float64 `<name>_<field>_residualize`: the residual of the field regressed, with an intercept, on the `against` fields over the rows of the group (neutralised: uncorrelated with each regressor within the group, mean 0). Needs `p + 2` complete rows in the group, else null; a constant / redundant regressor is left out. Block-level `excludeSelf: true` = leave-one-out (fitted on the other rows). The residual **plus** the raw field lets a model rebuild `against`: if `against` is kept out of the features on purpose (a baseline used as offset / initial score), emit the residual without the raw field (`output.exclude` for a computed field, `output.passThrough: keys \| none` for an input field — `exclude` never drops a pass-through input) |
| `ratingProb` | numeric `field` (a strength: a rating's `mu`, a team's `team_mu`), `sigma: <column>` (the row's uncertainty — the matching `sigma` readout; optional, 0 without; one `field` per op when named), `beta` (required, > 0: the rating's performance noise, `sigma / 2` of its prior by default = 25 / 6) | float64 `<name>_<field or as>_ratingProb`: the probability the rating model gives the row in its group — the Plackett–Luce contest read forward, `exp(mu / c)` normalised over the group with `c² = Σ (sigma² + beta²)` over the group's rows; a row without a finite `mu` / `sigma` is null and out of the contest; same scale as a market share (`ln(p_rating / p_market)` is the rating's disagreement with the market) |
| `harville` | numeric `field` read as win probabilities (normalised over the group), `top: [2, 3]` (places 1..3, default `[2, 3]`), `discount: [λ2, λ3]` (exponents applied when the 2nd / 3rd place is drawn; default 1, below 1 flattens the later places), `maxGroupSize` (default 64) | float64 `<name>_<field or as>_harville_top<k>`: the probability of finishing within the first k places (Harville forward computation). Null / negative → null, 0 can only lose; cubic in the group size for `top: 3` — a larger group than `maxGroupSize` reads null. Plain Harville overstates the favourites' later places; `discount: [0.81, 0.65]` is a measured starting point for fields of 8–18, the best exponents depend on the field size |
...
| `runLength` | `field`, `value` (numbers compare as numbers: `1` = 1.0) | `..._runlength` int64 |
...
| `rating` | `field` (the contest's numeric outcome), `context` (a `contexts[].name`: the rows of one group at one event time are a contest), `order: ascending` (default — a rank, smaller is better) or `descending` (a score), `method: plackettLuce` (default), `bradleyTerry`, `elo` or `gaussian` (the margin-of-victory update: the outcome is a continuous score and every strength of the contest is conditioned at once on the outcomes up to a common shift — one observation per entry, no `pairs`; `sigma` and `beta` required, in the outcome's units — `sigma: 1, beta: 0.5` for a standardised margin; `mu` defaults to 0), `funcs: [mu, sigma, count, delta, deviation, z]` (default `[mu, sigma]`; elo `[mu]`, no `sigma`; `deviation` = `mu` net of the prior, `z` = `mu` standardised within the pool of rated players — per `$self` pool and, in a team, per member entity; null until two are rated), priors `mu` (25; elo 1500) / `sigma` (`mu / 3`) / `beta` (`sigma / 2`) / `tau` (`sigma / 100`, the drift per contest) — or with `tauPer: P30D` the drift per that much time since the player's previous contest (`tau` must then be declared; the `sigma` read carries the drift up to the row, so a returning player reads wide — size `tauPer` well above the outcome's availability lag, which is a floor under every row's Δt), `pairs: all \| adjacent \| mean` (`bradleyTerry` / `gaussian` only: every opponent / the rank neighbours / the mean over the opponents), `with: [<entity> \| {entity, mu, sigma, tau}]` + `as` (rates the row as a TEAM — the block's entity plus other entities of the same row; strength = the members' sum, a contest's change shared by variance, so an entity is no longer rated for the company it keeps; columns `<name>_all_<as>_<func>` for the block's entity, `<name>_all_<as>_<member>_<func>` per member; not elo), `team: [mu, sigma, count, deviation]` (`<name>_all_<as>_team_<func>`: the row's whole strength — the identified quantity —, `count` the contests this exact team ran, `deviation` the sum net of the priors), elo `kFactor` (32) / `scale` (400) | `<name>_all_<field>_rating_<func>` (or `<name>_all_<as>_<func>`) float64, `count` int64. The block's `entity` is the rated player; every contest moves all its players at once (opponent-adjusted strength). Never rated = the prior with `count` 0. Runs as ONE replay under the global key (hint `sequence.rating.globalKey`); no `maxAge` / `maxEvents`, and the only filter is `"f = $self.f"` on a pre-event field, which splits the contests into independently replayed pools. Shifted by the outcome's availability; `minInterval` does not apply. `fit: {artifact: {uri, refit, require}}` on the block (not inherited from the top level) snapshots every pool's state after the replay (`<uri>/<planHash>/<block>.rating/`); a run whose input starts after the snapshot continues from it (serving: input = the rows to serve + every contest since), one reaching back to it replays from scratch and rewrites it; `refit` replays whatever the input, `require` fails a pool without a snapshot instead of replaying from the prior. Feed it to a context block (`zscore`, `gapToBest`) for the strength relative to the field; a scale-free one (`zscore`, `rank` — not `gapToBest`, which is in `mu` units) is also what makes it comparable over the warm-up stretch (every player starts from the prior, so the spread of `mu` grows until the pool has warmed up). Large fields (more than ~8 players): `bradleyTerry` sums each player's `k − 1` pairs, so one contest moves `mu` by several prior sds and collapses `sigma` (8.33 → 1.9 at 16 players; a larger `beta` softens it without curing it) — use `pairs: mean` (a contest weighs like one game; `sigma` narrows a few percent per contest) or `pairs: adjacent`, or `plackettLuce` / `elo`; `plackettLuce`'s `sigma` barely shrinks there whatever `beta` is — read `count`, not `sigma`, for how well a player is known; `gaussian` also adds up the `k − 1` pairs (and multiplies their kept variance fractions, `0.6^(k − 1)` for fresh equals: `sigma` × 0.02 at 16) — `pairs: mean` above a few entries |
| `aggregate` | `field` / `expr`, `funcs: [count, mean, avg, sum, std, min, max, first, last, rate]`; no field + `funcs: [count]` = COUNT(1). Optional `weightBy: "<numeric expr>"` — the past event's fields by name, the current row's as `$self.<field>` (a similarity kernel, e.g. `exp(-abs(start_price - $self.start_price) / 50)`; a string operand is compared by identity, `c == $self.c ? 1 : 0.25`; a `weightBy` on the block is the default of its aggregate ops): `count` = Σw (float64), `sum` = Σw·x, `mean` = Σw·x / Σw, `std` weighted; no `min / max / first / last`; null / NaN / ≤ 0 weights contribute nothing; `$self` fields must be known at `predictAt`; always scanned per row, so bound the window (`maxAge` / `maxEvents`); use `as:` when the block also has the plain aggregate of the field | `..._<func>`; count int64 (float64 under `weightBy`), mean / std / sum / rate float64, min / max / first / last input type |
