...
- name: enc
  ...
  shrinkage:
    ...
    leaveNodeOut: true                           # the leaf's rows leave every ancestor; leaf = the deepest level that has rows (an unseen / null-keyed leaf backs off, and the level it backs off to is what is subtracted)
