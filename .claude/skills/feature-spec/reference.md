...
| `ratingProb` | numeric `field` (a strength: a rating's `mu`, a team's `team_mu`), `sigma: <column>` (the row's uncertainty — the matching `sigma` readout; optional, 0 without), `beta` (required, > 0: the rating's performance noise, `sigma / 2` of its prior by default = 25 / 6) | float64 `<name>_<field or as>_ratingProb`: the probability the rating model gives the row in its group — the Plackett–Luce contest read forward, `exp(mu / c)` normalised over the group with `c² = Σ (sigma² + beta²)` over the group's rows; a row without a finite `mu` / `sigma` is null and out of the contest; same scale as a market share (`ln(p_rating / p_market)` is the rating's disagreement with the market) |
