...
| `transitionStats.parameters` / `spectralEmbedding.parameters` | error | an unknown key or entry in `sequenceOf` / `cooccur` / `emit` / `blend` (e.g. `emit: [{fromValueProb: x}]`), or a parameter of the *other* sequence-of-values type (`cooccur` / `of` / `maxValues` / `rank` on `transitionStats`; `order` / `emit` / `blend` on `spectralEmbedding`) — the message lists the accepted forms |
