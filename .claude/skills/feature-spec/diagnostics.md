...
| `fit.fold.until.crossFit` | info | under `until` the training rows get cross-fit values and the later rows forward ones: within a key a cross-fit value moves against the row's own outcome (leave-one-out leak, strongest for keys whose total barely changes), so a model trained on the training rows and evaluated on the later ones sees two distributions — use `fit.mode: forward` when the column must behave the same on every row |
