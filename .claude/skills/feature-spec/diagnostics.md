...
| `sequence.rating.context` / `.method` / `.order` / `.func` / `.parameter` | error | `rating` needs `context` (a `contexts[].name`); method is elo / bradleyTerry / plackettLuce / gaussian; order ascending / descending; funcs mu / sigma / count / delta (`sigma` not under elo); `sigma` / `beta` / `tau` belong to the Bayesian methods, `kFactor` / `scale` to elo; `sigma` / `beta` / `kFactor` / `scale` > 0, `tau` ≥ 0 (declare `sigma` when `mu` is 0) |
...
| `sequence.rating.with` | error / info | a team rating: `with` lists `entities[].name` other than the block's entity, each once (no entity called `team`), members take `entity` / `mu` / `sigma` (> 0) / `tau` (≥ 0) only; a team needs `as:` and one field, `plackettLuce` or `bradleyTerry` (elo keeps no variance to share a change by), and `team:` reads `mu` / `sigma` of a declared team. As an info it describes the team — nothing to fix |
...
| `sequence.rating.gaussian.units` | warning | `method: gaussian` reads the outcome as a margin, so `mu` / `sigma` / `beta` are in the outcome's units; `sigma` / `beta` was left to its default, which derives from the prior `mu` (`sigma = |mu| / 3`, `beta = sigma / 2`; 25 / 8.33 / 4.17 when `mu` is defaulted too), not from the outcome's spread — declare `mu` (a typical outcome), `sigma` (the spread of strengths), `beta` (the noise of one outcome) |
