...
| `transitionStats.emit` / `.order` / `.blend` / `.maxFeatures` | error | `emit: [distribution \| {toValueProb: <value>}]` is required; `order` 1..4; `blend.priorWeight` a positive number; `emit` must not produce more columns than `maxFeatures`; `emit: expected` needs an integer-coded field |
