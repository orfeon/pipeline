...
| `sequence.rating.context` / `.method` / `.order` / `.func` / `.parameter` | error | `rating` needs `context` (a `contexts[].name`); method is elo / bradleyTerry / plackettLuce; order ascending / descending; funcs mu / sigma / count / delta / deviation / z (`sigma` not under elo); `sigma` / `beta` / `tau` belong to the Bayesian methods, `kFactor` / `scale` to elo; `sigma` / `beta` / `kFactor` / `scale` > 0, `tau` ≥ 0 (declare `sigma` when `mu` is 0) |
...
| `sequence.rating.with` | error / info | a team rating: `with` lists `entities[].name` other than the block's entity, each once (no entity called `team`), members take `entity` / `mu` / `sigma` (> 0) / `tau` (≥ 0) only; a team needs `as:` and one field, `plackettLuce` or `bradleyTerry` (elo keeps no variance to share a change by), and `team:` reads `mu` / `sigma` / `count` / `deviation` of a declared team. As an info it describes the team — nothing to fix |
