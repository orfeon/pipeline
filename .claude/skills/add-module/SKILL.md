...
- **Registered name**: lowercase, short (e.g. `bigquery`, `select`, `pubsub`). This is what users write in
  ...
  looking up docs). Action services are configured as `action.<service>` (e.g. `action.bigquery`) and are
  placeable in any of the `sources`/`transforms`/`sinks` sections.
...
- Action → [action.md](action.md) — actions implement the `Action` SPI instead of extending a module
  base class, so Steps 2–4 below apply in adapted form; the guide is self-contained.
...
`Action` SPI contract shown in [action.md](action.md) instead):
...
2. Register in `src/main/resources/server/docs/module/index.yaml`: add `title` / `description` / `tags`
   under the matching `sources:` / `transforms:` / `sinks:` / `actions:` key (action entries use
   `title: action.<service>`). Write a real description (1–3 sentences, capabilities + supported
