...
- Pure models used by both layers: `Shrinkage` (lattice parse + row-local top-down composition,
  ...
  *reused* family, engine doc §9.6.3), `Spectral` (`type: spectralEmbedding`: unordered pair counts of a value with its lag
  ...
  logs after a key's replay (`SequenceEvaluator.ratingSummaries`). **Snapshot** (`RatingSnapshot`): the block's own `fit.artifact` (never the top-level one) → coordinates `artifact` / `artifactRefit` / `artifactRequired` on every readout column (outside the plan hash) → `RatingSnapshot.specsOf(stageColumns)` in `Wiring.applyStage` (which rejects a non-global window: one file per pool, one replay per window) → `KeyedHistoryDoFn.loadSnapshots` AT THE FIRST ROW of a key: a snapshot is continued from only when the first row is after its `foldedUntilMillis` — an input reaching back (a backfill, a RETRY of the key that wrote the file) replays from scratch and `writeSnapshots` rewrites it, which is what makes retries safe without any run id (a retry sees the same first row); `required` + missing → `MissingSnapshotException` → `failKey`; `foldedUntilMillis` makes `advanceRating` skip the runs up to it and count rows whose near edge precedes it (`rowsBeforeSnapshot` → counter `feature/ratingSnapshot_<stateKey>_rowsBefore`). One streamed JSON file per (stateKey, pool): `<uri>/<version>/<block>.rating/<stateKey>.<sha256(key)>.json` (players / teams / pools / foldedUntil written with `JsonWriter`, read with `JsonReader` — no tree or string of the whole pool on the heap). `gaussian` is the margin-of-victory method: every strength of the contest conditioned at once on the outcomes up to a common shift, closed form with w_i = 1/(v_i + β²): Ω_i = v_i w_i (r_i − r̄_w), Δ_i = v_i w_i (1 − w_i/Σw) (two entries = the pairwise margin update; a field = one observation per entry, never k − 1 pairs — a pairwise sum over-counts, 16 equals kept 2% of sigma); sigma and beta REQUIRED (error `sequence.rating.gaussian.units`), mu defaults to 0, no pairing (the compiler rejects `pairs`). Its own loop, so the frozen player arithmetic of bradleyTerry stays untouched. `Pairs` (`all` / `adjacent` /
...
- `VarianceComponents` — per-level (n, Σy, Σy²) per key as a Beam `Combine`, λ = σ²/τ² by the method
  ...
  the coordinates `foldBy` / `purgeBlocks` / `embargoBlocks` — and, under `fit.fold.until`, `untilBlock` /
  `forwardLagMillis` (`FeaturePlanCompiler.timeFoldCoordinates`, purge
  defaulting to the target label's horizon via `labelHorizon`, the lag from the shared `availabilityLag` helper) into
  `FitLevel.timeFold` (`isTimeFold()`; `Forward` /
  ...
  Under `fit.fold.until` the totals and the excluded range are clipped to `untilBlock`, a row of a later block
  (`TimeFold.isEvaluation`) reads the prefix up to `ForwardBlocks.usableBlock` instead (walk-forward, no window and no
  `minBlocks`), λ comes from the training period (`_TimeFoldTrainingTotals` → `_TimeFoldVc`; the artifact keeps the
  whole-input `_TimeFoldTotals`), and `auditTimeFold` counts against the training span.
