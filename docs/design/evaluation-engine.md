...
- **Align** calls `EvaluationScorer.prepare` (which also counts the unit's duplicate identities — adjacent
  ...
  are expanded only when a key's pending contributions exceed `PENDING_MAX`, in the bundle map, in the
