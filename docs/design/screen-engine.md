...
| `WindowQuantiles` | the window's sketches, one per sketched x column (`rank` / `median` / `min` / `max` / `quantile` / `edges` by column): the rank / absdev reference of independent rows, the value-bin edges of DSL §6.1 and a pair grid's edges; and the categorical candidates' exact level counts (`updateLevels`, `levels(c, maxLevels)` = the `Levels` dictionary: the most frequent named, the rest folded, DSL §6.2; a column past `LEVELS_HARD_CAP` fails); coder + `Fn` (input = accumulator = output) | coder + CombineFn |
