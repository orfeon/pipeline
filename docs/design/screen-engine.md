...
| `WindowQuantiles` | the window's sketches, one per sketched x column (`rank` / `median` / `min` / `max` / `quantile` / `edges` by column): the rank / absdev reference of independent rows, the value-bin edges of DSL §6.1 and a pair grid's edges; and the categorical candidates' exact level counts (`updateLevels`, `levels(c, maxLevels)` = the `Levels` dictionary: the most frequent named, `(null)` named outside the count, the rest folded, DSL §6.2; a column past `LEVELS_HARD_CAP` fails); coder + `Fn` (input = accumulator = output) | coder + CombineFn |
...
| `ConditioningScorer` | per-unit conditioning computations: `moments`, `initialTheta`, `design`, `fitted` and `evaluate` (`[n, ll, g, G]`, both delegating to `GlmFit`), `partial` (`[s, b, a]` per column, per period too, plus the gaussian variance sums and the fit's `[n, g, G]` per period under `FIT_PERIOD_KEY`; for the binned block `[s (B), H (B² / B), A (B × k)]` without period slices, kept twice — the window's, then the discovery half's — with `suggestions` (`binnedPartial(…, split)`); with `joint` the joint columns' sums at p̂ and their cross terms with F̃ under `JOINT_PARTIAL_KEY` (`addJointPartial`, laid out by `JointPartialLayout`); for a declared pair the product of two standardised design columns — a placebo pair the first member times a noise column — as one more `[s, b, a]` column under `pairKey`, with its period slices (no level slice); for a real pair its 2-D grid at the fitted means under `pairGridKey` (`pairGrid`: the members' raw values binned by their sketch edges, the K = k² cells as a one-hot block); the categorical candidates' blocks and their placebos under the marginal keys (`categoricalBlocks`)) | no |
...
| `ScreenReport` | `stats` per slot array, `binnedStats` / `blockChi2` (the block test: active bins, one reference dropped, Cholesky on the reduced system, χ²(df)), `gammas` + `partial` (the orthogonalisation) and `blockPartial` (the block's Γ, S⊥, H⊥, r²_F = 1 − tr H⊥ / tr H), `heterogeneity` (the level slices' Σ S_l² / H_l − (Σ S_l)² / Σ H_l, marginal from the score slices and partial from the partial slices), `suggestions` (the one-candidate recipes from the binned sums — marginal, or under conditioning the partial halves through `orthogonalBlock` / `partialBlock` with the window's Γ: `contrastChi2` along a bin-constant contrast, `shapes`, `isotonic`, the discovery / confirmation halves of the doubled `extra` vector, per-kind placebo cuts, `basis` / `r2_F` per record; `Bins` = the representatives / edges callbacks the finalize step builds from the sketches), `joint` (the several-candidate suggestions from the joint sums on the marginal or the partial basis — `marginalJoint` / `partialJoint`, the latter orthogonalising S / H / M against F in closed form — `JointSums`: the centred S / H / M, the pHd eigenpairs through `SymmetricEigen`, the redundancy clusters, the forward selection, the composite, and the differences / ratios from every pair's 2 × 2 Newton direction with the sketch minima for positivity), `interactions` (a real pair's shape from its 2-D grid: the best depth-2 tree of split gains, its share of the grid's block χ², the sides' asymmetry, DSL §8.7), `groupings` (a categorical candidate's levels cut once by effect, the strong levels as one-hot, DSL §6.2), `build` (records + summary + suggestions, the placebo cut per statistic kind — df1 / binned / het / pair; the pair records from the pair keys' partial sums through the same `gammas` / `partial`), `selection` (the pass list, `passedPairs` apart from `columns`), the output schemas, `describe` | no |
...
- **Partial pass** (one pass): at the fitted p̂, `[s, b, a]` per column × transform into a bundle-local map of
  ...
  `ScreenReport.blockPartial` solves its Γ (k × B) by the same multi-right-hand-side `solveGram` (`blockGamma`),
  forms S⊥ / H⊥ (`orthogonalBlock`) and takes χ² = S⊥' H⊥⁺ S⊥ over the bins the marginal block kept
  (`partialBlock`, DSL doc §6.1). With `suggestions` the block's partial vector is doubled — the window's sums,
  ...
  every column × transform key, with its period slices (a grouped unit's period, a row family's cells' periods)
  but no level slice; the report treats its key like any column's in `gammas` / `partial` / `partialPeriod`
  (DSL doc §8.6).
...
in-screen expansion (a built-in baseline-bin modifier for the heterogeneity test —
