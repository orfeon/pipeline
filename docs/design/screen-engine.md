...
| `ScoreAccumulator` | 10 slots (`S`, `H`, `N_OBS`, `C1..C6`, `X2` = the grouped raw second moment behind the relative degenerate check) for the window plus the same per period, min / max time; the bookkeeping key reuses the slots for run counts; custom coder; `Fn` (input = accumulator = output) | coder + CombineFn |
